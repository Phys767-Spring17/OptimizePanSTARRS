...
double power(double freq, double *params);
...
main()
{
   ...
   system("wc data.dat > wc.dat");
   wc=fopen("wc.dat","r");
   fscanf(wc,"%d",&Ndat);
   fclose(wc);
   params=calloc(Nparams,sizeof(double));
   scanf("%lg %lg",&params[0],&params[1]);
...
/* Now we have read in the data and declared the arrays.  We need to compute the correlation matrix for the noise and the candidate signal and add them together to get the correlation matrix for the model.  We then compute and output the log likelihood. */
...
double power(double freq, double *params)
{
/* Given a model for the power density and values of the model's parameters, compute the power density at frequency freq. */
...
/* Exchanges of rows multiply the determinant by -1, but in this specific case the determinant had better be positive.  Thus we ignore the sign flips. */
